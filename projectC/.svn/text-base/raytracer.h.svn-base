...
typedef enum { FRONT = 0, LEFT = 1, RIGHT = 2, UP = 3, DOWN = 4 } face; 
typedef struct {
...
cube cube_expr(location front_top_left, double length, 
		color(*sc)(object*,location), color sh); 
 ...
 * and corner (of theoretical psoter) as out parameters
 */ 
void cube_face_int_params(face f, cube c, direction* s_norm, 
	double* d, location* corner); 
...
/* Given scene, ray, hit_test, and object, returns the final 
 * computation of shadow and shining 
 */ 
color shining(scene s, ray r, hit_test h, object o); 
...
/* makes horizontal stripes texeure */ 
